...
def run(conf_path=Path(__file__).parent / 'config.yml'):
    ...
    for datapath in conf_path:
        ...
        #
        # When you create your own argument tuple, you need to drop the keywords and just use
    ...
    # Start a multiprocessing pool and use as many cores as available to run the requested simulations
    ...
    for i, sim in enumerate(filename_list):
        ...
        # Copy the config file to the output directory so this simulation is reproducible
        copy2(sim,p)
        ...
        # We need the Kerr parameter in order to convert to cartesian from Boyer-Lindquist coords
        a = a_list[i]
        ...
        # If you prefer not to work with h5 files, uncomment this block and comment out the h5py block above
        """
