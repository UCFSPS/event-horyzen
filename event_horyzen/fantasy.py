...
(e.g., 'M' and 'a' for the Kerr metric in Boyer-Lindquist coordinates) and a
list called Coord, which contains the coordinates (e.g., 'r' and 't' for the
Kerr metric in Boyer-Lindquist coordinates). In order to set up a metric,
...
def g11(Param,Coord):
    return (Param[1]**2.-2.*Param[0]*Coord[1]+Coord[1]**2.)/
           (Coord[1]**2.+Param[1]**2.*cos(Coord[2])**2.)

Extra step) To make your code more readable, you can redefine variables in
place of Param[i] or Coord[i], for example, the g11 function can be rewritten as:
def g11(Param,Coord):
    M = Param[0]
    a = Param[1]
    r = Coord[1]
...
important to find an omega that is appropriate for the problem at hand. The
easiest way to choose an omega is through trial and error:

Step 1) Start with omega=1; if you are working in geometric/code units in which
        all relevant factors are ~unity, this is usually already a good choice
        of omega
Step 2) If the trajectory varies wildly with time (this indicates highly
        chaotic, non-integrable behavior), increase omega and redo
        integration
...
spaces, q0 is a list containing the initial position, p0 is a list containing
the initial momentum, Param is a list containing the fixed parameters of the
metric (e.g., [M,a] for Kerr metric in Boyer-Lindquist coordinates), and order
...
output[timestep][0] = a list containing the position of the particle at said
                      timestep in the 1st phase space
output[timestep][1] = a list containing the momentum of the particle at said
                      timestep in the 1st phase space
output[timestep][2] = a list containing the position of the particle at said
                      timestep in the 2nd phase space
output[timestep][3] = a list containing the momentum of the particle at said
                      timestep in the 2nd phase space

As long as the equation of motion is integrable
(see section "A Guide on Choosing omega"), the trajectories in the two phase
...
def g00(Param, Coord):
    ...
    r = Coord[1]
    theta = Coord[2]
    Delta = r ** 2.0 - 2.0 * M * r + a ** 2.0 + Q ** 2
    rhosq = r ** 2.0 + a ** 2.0 * cos(theta) ** 2.0
    return (
        -(r ** 2.0 + a ** 2.0 + 2.0 * M * r * a ** 2.0 * sin(theta) ** 2.0 / rhosq)
        / Delta
    )
...
def g33(Param, Coord):
    ...
    Delta = r ** 2.0 - 2.0 * M * r + a ** 2.0 + Q ** 2
    rhosq = r ** 2.0 + a ** 2.0 * cos(theta) ** 2.0
    return (1.0 / (Delta * sin(theta) ** 2.0)) * (1.0 - 2.0 * M * r / rhosq)
...
def AutoJacob(Param, Coord, i, wrt):
    ...
    if i == 0:
        if wrt == 0:
            return dif(
                lambda p: CoordTrans0(Param, [p, point_1, point_2, point_3]), point_d
            )
        elif wrt == 1:
            return dif(
                lambda p: CoordTrans0(Param, [point_0, p, point_2, point_3]), point_d
            )
        elif wrt == 2:
            return dif(
                lambda p: CoordTrans0(Param, [point_0, point_1, p, point_3]), point_d
            )
        elif wrt == 3:
            return dif(
                lambda p: CoordTrans0(Param, [point_0, point_1, point_2, p]), point_d
            )

    if i == 1:
        if wrt == 0:
            return dif(
                lambda p: CoordTrans1(Param, [p, point_1, point_2, point_3]), point_d
            )
        elif wrt == 1:
            return dif(
                lambda p: CoordTrans1(Param, [point_0, p, point_2, point_3]), point_d
            )
        elif wrt == 2:
            return dif(
                lambda p: CoordTrans1(Param, [point_0, point_1, p, point_3]), point_d
            )
        elif wrt == 3:
            return dif(
                lambda p: CoordTrans1(Param, [point_0, point_1, point_2, p]), point_d
            )

    if i == 2:
        if wrt == 0:
            return dif(
                lambda p: CoordTrans2(Param, [p, point_1, point_2, point_3]), point_d
            )
        elif wrt == 1:
            return dif(
                lambda p: CoordTrans2(Param, [point_0, p, point_2, point_3]), point_d
            )
        elif wrt == 2:
            return dif(
                lambda p: CoordTrans2(Param, [point_0, point_1, p, point_3]), point_d
            )
        elif wrt == 3:
            return dif(
                lambda p: CoordTrans2(Param, [point_0, point_1, point_2, p]), point_d
            )

    if i == 3:
        if wrt == 0:
            return dif(
                lambda p: CoordTrans3(Param, [p, point_1, point_2, point_3]), point_d
            )
        elif wrt == 1:
            return dif(
                lambda p: CoordTrans3(Param, [point_0, p, point_2, point_3]), point_d
            )
        elif wrt == 2:
            return dif(
                lambda p: CoordTrans3(Param, [point_0, point_1, p, point_3]), point_d
            )
        elif wrt == 3:
            return dif(
                lambda p: CoordTrans3(Param, [point_0, point_1, point_2, p]), point_d
            )
...
def Hamil_inside(q, p, Param, wrt):
    ...
    return (
        ...
        + p[2] * p[2] * dm(Param, q, "g22", wrt)
        + p[3] * p[3] * dm(Param, q, "g33", wrt)
        + 2 * p[0] * p[1] * dm(Param, q, "g01", wrt)
        + 2 * p[0] * p[2] * dm(Param, q, "g02", wrt)
        + 2 * p[0] * p[3] * dm(Param, q, "g03", wrt)
        + 2 * p[1] * p[2] * dm(Param, q, "g12", wrt)
        + 2 * p[1] * p[3] * dm(Param, q, "g13", wrt)
        + 2 * p[2] * p[3] * dm(Param, q, "g23", wrt)
    )
...
def phiHC(delta, omega, q1, p1, q2, p2, Param):
    ...
    q1_updated = 0.5 * (
        q1
        + q2
        + (q1 - q2) * np.cos(2.0 * omega * delta)
        + (p1 - p2) * np.sin(2.0 * omega * delta)
    )
    p1_updated = 0.5 * (
        p1
        + p2
        + (p1 - p2) * np.cos(2.0 * omega * delta)
        - (q1 - q2) * np.sin(2.0 * omega * delta)
    )
...
def updator(delta, omega, q1, p1, q2, p2, Param):
    ...
    first_HA_step = np.array(
        [
            q1,
            phiHA(0.5 * delta, omega, q1, p1, q2, p2, Param)[1],
            phiHA(0.5 * delta, omega, q1, p1, q2, p2, Param)[0],
            p2,
        ]
    )
...
def geodesic_integrator(N, delta, omega, q0, p0, Param, order=2):
    ...
    for count, _ in enumerate(range(N)):
        if order == 2:
            updated_array = updator(
                delta, omega, result[0], result[1], result[2], result[3], Param
            )
        elif order == 4:
            updated_array = updator_4(
                delta, omega, result[0], result[1], result[2], result[3], Param
            )
        else:
            print("Order must be -2 or -4.")
            sys.exit(-1)
